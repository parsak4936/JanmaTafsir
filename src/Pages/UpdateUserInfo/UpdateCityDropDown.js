...
import  { Show500Errors, ShowNetorkErrors, ShowTokenErrors,Show400Errors } from '../../Components/ShowErrors/ShowErrors';
  

const UpdateCityDropDown = () => {
  const selectedStateID = useSelector(
    (state) => state.UpdateUserDataReducer.stateId
  );
  ...
  {selectedCity=='' ? <>یک شهر انتخاب کنید</>:<>   {props.value}</>}            </span>
    ...
    const oncityChange = (e) => {
        ...
        dispatch(
         allActions.userActions.UpdateSelectCity({
...
export default UpdateCityDropDown;